---
title: "Pacific cod obs coverage draft"
...
fig1dat%>%
  ...
  arrange(Area, Year) %>%
  flextable() %>%
  set_caption(caption=paste0("Total ", params$name, " catch (mt) by year, area, and gear. Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")) 
...
fig5dat%>%
  ...
  mutate(Year=as.factor(Year)) %>%
   arrange(Area, Year) %>%
  flextable() %>%
  set_caption(caption=paste0(params$name_cap, " lengths measured by year, area, and gear. Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY)."))
```
...
fig6dat%>%
  ...
  left_join(osum, by=c("Area"="mgmt_area", "Year"="year")) %>%
    mutate(Year=as_factor(Year)) %>%
     arrange(Area, Year) %>%
  flextable() %>%
  set_caption(caption=paste0(params$name_cap, " number of otoliths obtained by year, area, and gear. Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY)."))

```
 
 

```{r warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.asp=1, fig.cap=paste0("Figure 1. ", params$name ," catch by gear type that was either retained, discarded, or the sum of retained and discarded by management area. Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")}
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.cap=paste0("Figure 2. Proportion of fixed gear ", params$name," catch in pots in the Gulf of Alaska (dashed line), the Bering Sea and Aleutian Islands (dotted line), and both combined (solid line). Jig gear is excluded.")}
#process data
fig2dat<-casdat%>%
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.asp=1.5, fig.width=6, fig.cap=paste0("Figure 3. ",params$name_cap," catch in each observer coverage category in the Observer Deploy and Declare System (ODDS), including fixed gear electronic monitoring (EM). This catch was not necessarily observed. Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY). Jig gear is excluded.")}
#set plotting order
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.asp=1.5, fig.width=6, fig.cap=paste0("Figure 5. The total and proportion of ",params$name," lengths collected by observers by gear and area. Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")}
#plot 
...
fig5_lnum <-ggplot(fig5dat,aes(x=as.factor(year),y=n_length/1000,fill=gear))+
  ...
  labs(y="Thousands lengths collected", x = "Year", title="Lengths collected")+
    scale_x_discrete(breaks = seq(startyear, endyear, by = 3))+
    scale_color_manual(values=gear_colors)+
  scale_fill_manual(values=gear_colors)+
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.asp=1.5, fig.width=6, fig.cap=paste0("Figure 6. The number of otoliths collected by observers and the proportion of ", params$name," ages by gear and area. Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")}
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.width=6,  fig.cap=paste0("Figure 7. The number of ",  params$name," lengths collected per ton of catch by management area (panel a) or by gear (panel b). Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")}
#mgmt area
fig7dat_a <- norln%>%
  group_by(year, mgmt_area) %>%
  summarize(n_length=sum(n_length)) %>% 
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.cap=paste0("Figure 8. The number of ",  params$name," otoliths collected per ton of catch by management area (panel a) or by gear (panel b). Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")}
#mgmt area
fig8dat_a <- norage%>%
  filter(type_1_otolith=="Y") %>% 
  group_by(year, mgmt_area) %>%
...
fig9a <- ggplot(fig9dat_a,aes(x=as.factor(year),y=n_length,fill=obs_source))+
  ...
  facet_grid(mgmt_area~.)+
    scale_color_manual(values=c("#BC4700","#4C9C2E","#00467f"))+
  scale_fill_manual(values=c("#BC4700","#4C9C2E","#00467f"))+
  scale_x_discrete(breaks = seq(startyear, endyear, by = 3))+
...
```{r eval=FALSE, include=FALSE, warning=FALSE, message=FALSE, echo=FALSE, fig.asp=1.5, fig.width=6, fig.cap=paste0("Figure 10. The proportion of ",  params$name," catch by observer coverage (left) and number of lengths collected per metric ton of catch (right). Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")}
...
fig10lnmt <- norln%>%
  ...
  by=c("year"="year", "mgmt_area"="mgmt_area"))%>%
  mutate(lratio=n_length/catch_mt,
         lratio_scaled=scale(lratio))

ggplot()+
  geom_line(data=fig10propEM, aes(x=year, y=prop_em_scaled), color="black")+
  geom_line(data=fig10lnmt, aes(x=year, y=lratio_scaled), color="red")+
  facet_wrap(~mgmt_area, ncol=1)+
  ylab("scaled lenghts/mt (red) and proportion EM (black)")+
  theme_doc()
...
fig10lnmt <- norln%>%
  filter(gear %in% c("POT","HAL") & year >=2018) %>%
  group_by(year, mgmt_area) %>%
  summarize(n_length=sum(n_length)) %>% 
  left_join(casdat%>%
              filter(gear %in% c("POT","HAL")) %>%
...
fig10dat<-fig10propEM %>%
  left_join(fig10lnmt, by=c("year"="year", "mgmt_area"="mgmt_area"))

ggplot()+
  geom_text(data=fig10dat, aes(x=prop_em, y=lratio, label=year), color="blue")+
  facet_wrap(~mgmt_area, ncol=1, scales="free")+
  stat_smooth(data=fig10dat, aes(x=prop_em, y=lratio), method="lm", se=FALSE)+
  ylab("lengths/mt")+
  theme_doc()
```

![Appendix 1. Map of Fisheries Management Plan subareas.](fmp_subarea.png)
