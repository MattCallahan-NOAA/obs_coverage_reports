...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.cap=paste0("Figure 2. Proportion of fixed gear ", params$name," catch in pots in the Gulf of Alaska, the Bering Sea and Aleutian Islands, and both combined")}
#process data
fig2dat<-casdat%>%
...
fig5dat%>%
  ...
  pivot_wider(id_cols=c(Year, Area), names_from=gear, values_from=n_length) %>%
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.asp=1.5, fig.width=6, fig.cap=paste0("Figure 6. The total and proportion of ", params$name," ages collected by observers by gear and area. Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")}
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.cap=paste0("Figure 8. The number of ",  params$name," ages collected per ton of catch by management area (panel a) or by gear (panel b). Gear types include pelagic trawl (PTR), non-pelagic trawl (NPT), Pot (POT), or Hook and Line (HAL). Areas include the Aleutian Islands (AI), Bering Sea (BS), Western Gulf of Alaska (WGOA), Central Gulf of Alaska (CGOA), West Yakutat (WY), and East Yakutat (EY).")}
#mgmt area
fig8dat_a <- norage%>%
  filter(type_1_otolith=="Y") %>% 
  group_by(year, mgmt_area) %>%
