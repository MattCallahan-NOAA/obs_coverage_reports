...
fig5dat%>%
  ...
  mutate(Year=as.factor(Year)) %>%
   arrange(Area, Year) %>%
  flextable() %>%
  set_caption(caption=paste0(params$name_cap, " lengths measured by year, area, and gear"))
```
...
fig6dat%>%
  ...
  left_join(osum, by=c("Area"="mgmt_area", "Year"="year")) %>%
    mutate(Year=as_factor(Year)) %>%
     arrange(Area, Year) %>%
  flextable() %>%
  set_caption(caption=paste0(params$name_cap, " number of otoliths obtained by year, area, and gear"))
...
```{r warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.cap=paste0("Figure 2. Proportion of fixed gear ", params$name," catch in pots in the Gulf of Alaska (dashed line), the Bering Sea and Aleutian Islands (dotted line), and both combined (solid line)")}
#process data
fig2dat<-casdat%>%
